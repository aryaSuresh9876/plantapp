    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        buttonPlaceYourOrder.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                onPlaceOrderButtonClick(PlantStoreModel);
            }
        });
        ...
        switchDelivery.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if(isChecked) {
                    ...
                    isDeliveryOn = true;
                    calculateTotalAmount(PlantStoreModel);
                } else {
        ...
        initRecyclerView(PlantStoreModel);
        calculateTotalAmount(PlantStoreModel);
    }

    private void calculateTotalAmount(PlantStore PlantStoreModel) {
        float subTotalAmount = 0f;

        for(Menu m : PlantStoreModel.getMenus()) {
            subTotalAmount += m.getPrice() * m.getTotalInCart();
        }

        tvSubtotalAmount.setText("$"+String.format("%.2f", subTotalAmount));
        if(isDeliveryOn) {
            tvDeliveryChargeAmount.setText("$"+String.format("%.2f", PlantStoreModel.getDelivery_charge()));
            subTotalAmount += PlantStoreModel.getDelivery_charge();
        }
        tvTotalAmount.setText("$"+String.format("%.2f", subTotalAmount));
    }

    private void onPlaceOrderButtonClick(PlantStore PlantStoreModel) {
        ...
        i.putExtra("PlantStoreModel", PlantStoreModel);
        startActivityForResult(i, 1000);
    }

    private void initRecyclerView(PlantStore PlantStoreModel) {
        cartItemsRecyclerView.setLayoutManager(new LinearLayoutManager(this));
        placeYourOrderAdapter = new PlaceYourOrderAdapter(PlantStoreModel.getMenus());
